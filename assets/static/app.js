document.addEventListener('DOMContentLoaded', () => {
    ...
    const closeAllModals = () => {
        [createFolderModal, uploadModal, deleteConfirmModal].forEach(closeModal);
        if(deletePasswordInput) deletePasswordInput.value = "";
        if(uploadSession.file && !uploadSession.isPaused && uploadSession.chunksCompleted < uploadSession.totalChunks) { }
    };
        ...
        fileList.innerHTML = items.map(item => {
            ...
            <div class="file-item" data-path="${item.path}" data-is-dir="${item.is_dir}" data-name="${item.name}">
                ...
                <div class="item-actions">
                    ${!item.is_dir ? `<button class="icon-button download" data-path="${item.path}">${icons.download}</button>` : ''}
    ...
    const resetUploadUI = () => {
        ...
        uploadSession = { file: null, customName: null, chunkSize: 10 * 1024 * 1024, totalChunks: 0, chunksCompleted: 0, isPaused: false, isMerging: false, fileId: null, path: '', activeConnections: 0, maxConcurrency: 4, nextChunkToIndex: 0 };
        ...
        if(pauseUploadBtn) pauseUploadBtn.disabled = false; // Re-enable pause
    };
    ...
    const performChunkUpload = async (index) => {
        ...
        // NEW: Tell the server WHO is uploading
        if (mySocketId) fd.append('socketId', mySocketId);
        ...
        try {
            ...
            if (data.success) {
                ...
                // --- CASE 1: SERVER SAYS "MERGING STARTED" (Async) ---
                if (data.merging) {
                    // Stop the upload loop immediately
                    uploadSession.isMerging = true; 
                    uploadSession.chunksCompleted = uploadSession.totalChunks;
                    
                    // Update UI to Merging State
                    progressBar.classList.add('progress-bar-merging');
                    progressBar.style.width = '100%';
                    progressText.textContent = '100%';
                    statusText.textContent = 'Merging on server... Please wait.';
                    statusText.style.color = '#238636';
                    if(pauseUploadBtn) pauseUploadBtn.disabled = true;
                    
                    // Do NOT call processQueue() here. We wait for SocketIO now.
                    return; 
                } 
                
                // --- CASE 2: SERVER SAYS "COMPLETED" (Small files / Sync merge) ---
                ...
                // --- CASE 3: NORMAL CHUNK SUCCESS ---
                    ...
                    processQueue(); // Continue uploading
                }
            ...
            // If merging started, ignore errors (timeouts are expected)
    ...
    if(startUploadBtn) {
        startUploadBtn.addEventListener('click', (e) => {
            ...
            uploadSession.isMerging = false; // Reset flag
    ...
    // Handle Reloads (for Admin dashboard)
    ...
    // NEW: Handle Upload Completion Signal
    socket.on('upload_status', (msg) => {
        console.log("Upload Status:", msg);
        if (msg.status === 'completed') {
            // This is the reliable signal we waited for!
            showToast('Upload Complete!');
            if(uploadModal && uploadModal.classList.contains('modal')) closeModal(uploadModal);
            resetUploadUI();
            if(fileList) fetchFiles(currentState.path);
